...
class CandidatePages:
    ...
    def __init__(self,ocr, keywords=["bezoldiging", "wnt"]):   
        self.ocr=ocr
    ...
    def get_candidates(self, pdf_path="Type2.pdf"):
        self.pages = self.extract_pages_as_strings(pdf_path)
        if not self.scannCheck(pages):
            pages = self.ocr.on_file(pdf_path)
...
if __name__ == "__main__":
    #pdf_path = os.path.join(os.getcwd(), "data", "Type3.pdf")
    #keywords = ["bezoldiging", "wnt"]
    candidatePages = CandidatePages()
    print(candidatePages.candidates)
    print(candidatePages.pages)
    print(candidatePages.candidates) if candidatePages.scannCheck(candidatePages.pages)  else print("Scanned PDF")
